...
class CadastroGUI:
    def __init__(self, root):
        ...
        self.frame_cadastro = ttk.LabelFrame(root, text="Cadastro de Funcionários")
        self.frame_cadastro.pack(pady=10)

        # Remova o campo de entrada para o ID
        self.labels = ["Matrícula", "Nome", "Data Nascimento", "Cargo", "Setor", "Salário"]
        self.entries = {}

        for label in self.labels:
            ttk.Label(self.frame_cadastro, text=label).grid(row=self.labels.index(label), column=0, sticky="w", padx=10, pady=5)
            self.entries[label] = ttk.Entry(self.frame_cadastro)
            self.entries[label].grid(row=self.labels.index(label), column=1, padx=10, pady=5)

        self.button_cadastrar = ttk.Button(root, text="Cadastrar Funcionário", command=self.cadastrar_funcionario)
